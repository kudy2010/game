...
import { CLASSES, PERKS, QUESTS, DUNGEONS, ITEMS, JOBS, QUEST_ENEMIES, STAMINA_MAX, STAMINA_REGEN_MS, BEER_MAX, BEER_RESTOCK_MS, xpFor, rng, T, scaleEnemy, getQuestEnemy, getStaminaCost } from './data.js';
import { computeStats, getDungeonLoot, getSellPrice, applyXpForLevelUp } from './utils.js';
import { S, ANIM_CSS } from './styles.js';
import { AM, SFX, MUSIC } from './audio.js';
import { PixelIcon, CharSprite, classIconKey } from './components/icons.jsx';
import { TravelScreen } from './components/TravelScreen.jsx';
import { CombatScreen } from './components/CombatScreen.jsx';
import { DungeonBetween } from './components/DungeonBetween.jsx';
import { DungeonReward } from './components/DungeonReward.jsx';
...
import { HeroTab } from './components/tabs/HeroTab.jsx';
...
import { TavernTab } from './components/tabs/TavernTab.jsx';
...
export function App() {
    ...
    useEffect(function () {
        window.storage.get("ps_global").then(function (r) {
            ...
            try {
                ...
                if (typeof g.musicVol === "number") { setMusicVol(g.musicVol); AM.setMusVol(g.musicVol); }
                if (typeof g.uiScale === "number") setUiScale(g.uiScale);
                if (typeof g.musicSong === "number") setMusicSong(g.musicSong);
            } catch (e) { }
        }).catch(function () { });
    }, []);
    useEffect(function () {
        ...
        globalSaveTimer.current = setTimeout(function () {
            try { window.storage.set("ps_global", JSON.stringify({ musicVol: musicVol, uiScale: uiScale, musicSong: musicSong })); } catch (e) { }
        }, 500);
    ...
    useEffect(function () {
        function unlock() {
            ...
            try { AM.resume(); } catch (e) { }
        }
        var optsOnce = { passive: true, once: true };
        var rootEl = null;
        try { rootEl = document.getElementById("root"); } catch (e) { }
        if (rootEl) {
    ...
    var saveNow = function (onDone) {
        ...
        try {
            ...
            if (res && typeof res.then === "function") { res.then(finish).catch(finish); } else { setTimeout(finish, 50); }
        } catch (e) { if (onDone) setTimeout(onDone, 50); }
    };
    useEffect(function () {
        ...
        var t = setTimeout(function () { setTravelState(function (prev) { if (!prev) return null; return Object.assign({}, prev, { timeLeft: prev.timeLeft - 1 }); }); }, 1000);
        return function () { clearTimeout(t); };
    }, [travelState]);
    useEffect(function () {
        if (!hero || !hero._pendingLevelUp) return;
        if (dungeonReward || dungeonBetween || combatData || travelState || levelUpState) return;
    ...
    useEffect(function () {
        ...
        function handler() { if (musicStarted.current) return; musicStarted.current = true; AM.setMusVol(musicVol); MUSIC.start(0); setMusicOn(true); }
    ...
    useEffect(function () {
        if (!hero) return;
        var id = setInterval(function () { setHero(function (prev) { if (!prev || prev.hp >= prev.maxHp) return prev; var heal = Math.max(1, Math.floor(prev.maxHp / 30)); return Object.assign({}, prev, { hp: Math.min(prev.maxHp, prev.hp + heal) }); }); }, 10000);
    ...
    useEffect(function () {
        var id = setInterval(function () {
            setBeerStock(function (b) { if (b >= BEER_MAX) return b; return b + 1; });
    ...
    var toggleMusic = function () { AM.resume(); if (musicOn) { MUSIC.stop(); setMusicOn(false); } else { MUSIC.start(musicSong); setMusicOn(true); } };
    var musicPrev = function () { MUSIC.prev(); setMusicSong(MUSIC.si()); if (!musicOn) setMusicOn(true); };
    var musicNext = function () { MUSIC.next(); setMusicSong(MUSIC.si()); if (!musicOn) setMusicOn(true); };
    ...
    var processXpGain = function (heroState, xpG) {
        ...
        if (result.luInfo) { result.hero._pendingLevelUp = result.luInfo; setTimeout(SFX.lvlUp, 300); }
        return result.hero;
    };
    var onCombatWin = function (remainHp) {
        ...
        if (cd.type === "quest") {
            ...
            if (loot) setInventory(function (inv) { return inv.concat([loot]); });
            if (gotPierogi) setPierogi(function (p) { return p + 1; });
    ...
    var onDungeonBetweenHeal = function (type, amount) {
        ...
        setDungeonBetween(function (prev) { if (!prev) return prev; return Object.assign({}, prev, { heroHp: Math.min(hero.maxHp, prev.heroHp + amount) }); });
    ...
    var onLevelUpConfirm = function (choice) {
        setHero(function (prev) {
            ...
            Object.keys(choice.statBonuses).forEach(function (stat) { if (choice.statBonuses[stat] > 0) next.stats[stat] = (next.stats[stat] || 0) + choice.statBonuses[stat]; });
            if (choice.perkId) {
                next.perks = (prev.perks || []).concat([choice.perkId]);
                var perk = PERKS.find(function (p) { return p.id === choice.perkId; });
                if (perk) {
                    if (perk.hpFlat) { next.maxHp = prev.maxHp + perk.hpFlat; next.hp = Math.min(next.maxHp, prev.hp + perk.hpFlat); }
                    if (perk.hpPct) { var inc = Math.floor(prev.maxHp * perk.hpPct); next.maxHp = prev.maxHp + inc; next.hp = Math.min(next.maxHp, prev.hp + inc); }
                }
            }
            return next;
        });
    ...
    var equipItem = function (item) {
        ...
        setInventory(function (inv) { var n = inv.filter(function (i) { return i.uid !== item.uid; }); if (prev) n = n.concat([prev]); return n; });
        notify(T("Założono ", "Equipped ") + item.name);
    };
    var unequipItem = function (slot) { var it = equipped[slot]; if (!it) return; setEquipped(function (e) { var n = Object.assign({}, e); n[slot] = null; return n; }); setInventory(function (inv) { return inv.concat([it]); }); };
    var buyItem = function (it) { if (gold < it.price) { notify(T("Za mało złota!", "Not enough gold!"), "err"); return; } setGold(function (g) { return g - it.price; }); setInventory(function (inv) { return inv.concat([Object.assign({}, it, { uid: Date.now() + Math.random() })]); }); notify(T("Kupiono ", "Bought ") + it.name + "!"); };
    var buyPotion = function (p) { if (gold < p.price) { notify(T("Za mało złota!", "Not enough gold!"), "err"); return; } setGold(function (g) { return g - p.price; }); var key = p.id === "p1" ? "small" : p.id === "p2" ? "medium" : "large"; setPotions(function (prev) { var n = Object.assign({}, prev); n[key] = (n[key] || 0) + 1; return n; }); notify(T("Kupiono ", "Bought ") + p.name + "!"); };
    ...
    var applySlotData = function (s) {
        ...
        if (typeof s.stamina === "number") setStamina(s.stamina); else setStamina(STAMINA_MAX);
        if (typeof s.pierogi === "number") setPierogi(s.pierogi); else setPierogi(0);
        if (s.activeJob) setActiveJob(s.activeJob); else setActiveJob(null);
        if (s.dailyLastClaim) setDailyLastClaim(s.dailyLastClaim); else setDailyLastClaim(0);
        if (s.lang) setLang(s.lang);
        if (typeof s.beerStock === "number") setBeerStock(s.beerStock); else setBeerStock(BEER_MAX);
        if (s.beerLastRestock) setBeerLastRestock(s.beerLastRestock); else setBeerLastRestock(0);
    };
    ...
    var deleteSave = function (slot) {
        ...
        if (activeSlot === slot) { setHero(null); setScreen("title"); }
        setHasSave(true);
    ...
    var drinkBeer = function () {
        if (beerStock <= 0 || pierogi <= 0 || stamina >= STAMINA_MAX) return;
        SFX.heal();
    ...
    var claimDaily = function () {
        var now = Date.now();
        if (now - dailyLastClaim < 24 * 60 * 60 * 1000) return;
    ...
    return (React.createElement("div", { style: { width: "100%", minHeight: "100vh", background: "#0d0801", display: "flex", justifyContent: "center", alignItems: "flex-start" } },
        React.createElement("div", { style: (function () {
                var canZoom = false;
                try { canZoom = !!(window.CSS && CSS.supports && CSS.supports("zoom", "1")); } catch (e) { }
                if (canZoom) { return Object.assign({}, S.wrap, { zoom: uiScale, width: "100%" }); }
            ...
            React.createElement("div", { style: S.mbar },
                ...
                React.createElement("span", { style: { flex: 1, fontSize: 10, color: "#9a7030", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginLeft: 2 } }, "🎵 ", MUSIC.SONGS[musicSong].name),
            ...
            React.createElement("div", { style: { height: 7, background: "#150e04", position: "relative", borderBottom: "1px solid #2e1e08", overflow: "hidden" } },
                ...
                React.createElement("div", { style: { position: "absolute", right: 5, top: "-1px", fontSize: 9, color: "#7a6030", lineHeight: "7px" } }, hero.xp, "/", hero.xpNeeded, " XP")),
