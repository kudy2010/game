...
var RARITY_COLOR = { common: "#c8a44a", rare: "#4499ff", epic: "#cc44ff", legendary: "#ffaa00" };
var RARITY_GLOW = { common: "none", rare: "0 0 8px #4499ff66", epic: "0 0 10px #cc44ff66", legendary: "0 0 14px #ffaa0088" };
var SLOT_EMOJI = { weapon: "⚔️", helmet: "🪖", armor: "🛡️", boots: "👢", accessory: "📿" };
var TIER_COLOR = { 1: "#9a7a30", 2: "#3a9a6a", 3: "#9a3ac8" };
var STAT_COLOR = { str: "#ff6655", agi: "#44ccdd", int: "#cc77ff", con: "#44dd88" };
var STAT_LABEL = { str: "⚔️ STR", agi: "🏃 AGI", int: "🧠 INT", con: "❤️ CON" };
function getQuestDiff(minLevel) {
    if (minLevel <= 1)
        return { label: "Easy", color: "#44dd88", bg: "#0a2010", pulse: false };
    if (minLevel <= 3)
        return { label: "Medium", color: "#f0c060", bg: "#2a1a04", pulse: false };
    if (minLevel <= 6)
        return { label: "Hard", color: "#ff8844", bg: "#2a1004", pulse: false };
    return { label: "Brutal", color: "#ff4444", bg: "#2a0404", pulse: true };
}
// ── COMBAT REACTIONS ──────────────────────────────────────────────────────────
var REACT_ATK = {
    hit: {
        hero: ["Masz, skurczybyku!", "Weź to!", "No i jak ci?", "Nie płacz.", "Zasłużone!", "I co teraz?"],
        enemy: ["Ha! Trafiłem!", "Masz!", "Cierp!", "Słabeusz!", "Ból jest życiem!"],
    },
    crit: {
        hero: ["ŚWIĘTA ARMATA!!!", "K***A, JAK TRAFIŁEM!", "To za wszystkie podatki!", "BOOM!!!", "MIAŻDŻĄCE!!!"],
        enemy: ["GRZMOT!", "KTO TERAZ PŁACZE?!", "PADNIJ!", "KONIEC DLA CIEBIE!", "Żegnaj, głuptasie!"],
    },
    miss: {
        hero: ["Kurwa... spudłowałem.", "No serio?!", "DLACZEGO?!", "Zaślepiony byłem.", "Ręce mi drżą..."],
        enemy: ["Jak to możliwe?!", "Pudło...", "Okulary bym potrzebował.", "Niee...", "Co to było?!"],
    },
};
var REACT_DEF = {
    dodge: {
        hero: ["Ho ho, nie dzisiaj!", "Zbyt wolny!", "Hehe, pudło!", "Widziałem to z daleka!"],
        enemy: ["Niezłe nóżki!", "Hehe, zwinny jesteś?", "Szczęściarz.", "Daleko nie uciekniesz!"],
    },
    block: {
        hero: ["Solidna obrona!", "Nie przejdziesz.", "Próbuj dalej, debi*!", "Żelazo, nie człowiek!"],
        enemy: ["Imponujące...", "Żelazna obrona...", "Niezłe.", "Hmm... twardy jesteś."],
    },
};
function pickReact(side, role, type) {
...
var CLASSES = {
    musketeer: { name: "Musketeer", emoji: "🔫",
        desc: "Swift and deadly from afar. Deals bonus damage with ranged weapons.",
        preferredType: "ranged", primaryStat: "agi", secondaryStat: "str",
        stats: { str: 4, agi: 8, int: 5, con: 3 } },
    knight: { name: "Knight", emoji: "⚔️",
        desc: "Unyielding steel on the battlefield. Deals bonus damage with physical weapons.",
        preferredType: "physical", primaryStat: "str", secondaryStat: "con",
        stats: { str: 8, agi: 3, int: 3, con: 8 } },
    alchemist: { name: "Alchemist", emoji: "⚗️",
        desc: "Explosive concoctions. Eyebrows optional. Deals bonus damage with magical weapons.",
        preferredType: "magical", primaryStat: "int", secondaryStat: "agi",
        stats: { str: 3, agi: 5, int: 10, con: 4 } },
};
var QUEST_ENEMIES = {
...
var ITEMS = [
    // ── Generic weapons (no dominant class) ──────────────────────────────────
...
function getQuestEnemy(id) { var p = QUEST_ENEMY_POOLS[id]; return p ? Object.assign({}, p[rng(0, p.length - 1)]) : Object.assign({}, QUEST_ENEMIES[id]); }
// Scale enemy HP and stats with hero level — starts having effect above level 3
function scaleEnemy(enemy, heroLevel) {
    if (!enemy || !heroLevel || heroLevel <= 3) return enemy;
    var lvlOver = heroLevel - 3;
    var hpMult  = 1 + lvlOver * 0.14;   // +14% HP per level above 3
    var stMult  = 1 + lvlOver * 0.06;   // +6% stats per level above 3
    var scaled  = Object.assign({}, enemy);
    scaled.hp   = Math.round(enemy.hp * hpMult);
    scaled.stats = {};
    Object.keys(enemy.stats).forEach(function(k) {
        scaled.stats[k] = Math.max(1, Math.round((enemy.stats[k] || 0) * stMult));
    });
    scaled._scaled = true;
    return scaled;
}
...
var DUNGEONS = [
    { id: 1, name: "Rusted Keep", emoji: "🏚️", minLevel: 1, travel: 10, lootTable: "rare", xp: 120, gold: [50, 100], desc: "Abandoned fortress. Rats with delusions of grandeur.",
        sections: [{ name: "Gatehouse", enemy: { name: "Gate Rat", emoji: "🐀", hp: 68, stats: { str: 6, agi: 4, int: 1, con: 4 }, desc: "A rat with a grudge." } }, { name: "Great Hall", enemy: { name: "Rusted Knight", emoji: "🤺", hp: 105, stats: { str: 8, agi: 6, int: 3, con: 7 }, desc: "Clanks ominously." } }, { name: "Keep Tower", enemy: { name: "Iron Golem", emoji: "🤖", hp: 165, stats: { str: 11, agi: 3, int: 1, con: 13 }, desc: "Slow but hits hard." }, boss: true }] },
    { id: 2, name: "Fort Ironwood", emoji: "🏰", minLevel: 4, travel: 15, lootTable: "epic", xp: 300, gold: [120, 220], desc: "A real fortification, defended by real soldiers.",
        sections: [{ name: "Outer Wall", enemy: { name: "Ironwood Guard", emoji: "💂", hp: 195, stats: { str: 13, agi: 7, int: 4, con: 11 }, desc: "Professional. Annoying." } }, { name: "Barracks", enemy: { name: "Sergeant Major", emoji: "⚔️", hp: 285, stats: { str: 15, agi: 8, int: 6, con: 13 }, desc: "Twenty years' experience." } }, { name: "Commander's Hall", enemy: { name: "Iron Commander", emoji: "👮", hp: 405, stats: { str: 18, agi: 10, int: 8, con: 15 }, desc: "Veteran of twelve campaigns." }, boss: true }] },
    { id: 3, name: "Powder Magazine", emoji: "💥", minLevel: 7, travel: 20, lootTable: "epic", xp: 520, gold: [200, 360], desc: "Do NOT shoot the walls. This cannot be overstated.",
        sections: [{ name: "Fuse Room", enemy: { name: "Powder Warden", emoji: "🧨", hp: 420, stats: { str: 18, agi: 11, int: 10, con: 13 }, desc: "Extremely flammable job." } }, { name: "Arsenal", enemy: { name: "Blast Captain", emoji: "💣", hp: 570, stats: { str: 21, agi: 13, int: 13, con: 15 }, desc: "Loves his grenades." } }, { name: "Core Chamber", enemy: { name: "Grand Bomb", emoji: "💥", hp: 750, stats: { str: 24, agi: 14, int: 17, con: 18 }, desc: "It breathes." }, boss: true }] },
    { id: 4, name: "Dragon's Keep", emoji: "🐉", minLevel: 10, travel: 30, lootTable: "legendary", xp: 1050, gold: [400, 720], desc: "The dragon has a cannon collection. You covet it.",
        sections: [{ name: "Dragon Gate", enemy: { name: "Drake Sentinel", emoji: "🦎", hp: 630, stats: { str: 24, agi: 14, int: 14, con: 21 }, desc: "A scaled wall with opinions." } }, { name: "Hoard Chamber", enemy: { name: "Dragon Knight", emoji: "🐲", hp: 870, stats: { str: 28, agi: 17, int: 17, con: 24 }, desc: "The dragon's champion." } }, { name: "Dragon's Throne", enemy: { name: "Powder Dragon", emoji: "🐉", hp: 1170, stats: { str: 34, agi: 20, int: 25, con: 27 }, desc: "Ancient. Explosive. Disappointed in you." }, boss: true }] },
];
// ── TRANSLATIONS ──────────────────────────────────────────────────────────────
var T = function (pl, en) { return (typeof window !== "undefined" && window.__LANG === "en") ? en : pl; };
// ── STAMINA ───────────────────────────────────────────────────────────────────
var STAMINA_MAX = 100;
var STAMINA_REGEN_MS = 3 * 60 * 1000; // +1 co 3 minuty
var STAMINA_COST = { 1: 10, 2: 10, 3: 15, 4: 22, 5: 22, 6: 30 }; // per quest id
function getStaminaCost(q) { return STAMINA_COST[q.id] || 15; }
// ── JOBS ──────────────────────────────────────────────────────────────────────
var JOBS = [
...
var BEER_MAX = 3;
var BEER_RESTOCK_MS = 45 * 60 * 1000;
// dmgType display helpers
var DMG_TYPE_LABEL = { physical: "⚔ Physical", ranged: "🎯 Ranged", magical: "✨ Magical" };
var DMG_TYPE_COLOR = { physical: "#e08844", ranged: "#44bbdd", magical: "#cc77ff" };
var CLASS_TAG_INFO = {
