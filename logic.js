...
function rollRound(atkS, defS, forceHit) {
    ...
    // +20% when weapon type matches class preference
...
function computeStats(hero, eq) {
    var s = Object.assign({}, hero.stats);
    s._class = hero.class;
    s._weaponDmgType = (eq && eq.weapon && eq.weapon.dmgType) ? eq.weapon.dmgType : null;
    Object.values(eq).forEach(function (it) { if (!it)
        return; Object.keys(it.stats).forEach(function (k) { s[k] = (s[k] || 0) + it.stats[k]; }); });
    (hero.perks || []).forEach(function (pid) { var p = PERKS.find(function (x) { return x.id === pid; }); if (p && p.sB)
        Object.keys(p.sB).forEach(function (k) { s[k] = (s[k] || 0) + p.sB[k]; }); });
    return s;
}
function getDungeonLoot(d) { var pool = DUNGEON_ITEMS.filter(function (it) { return it.rarity === d.lootTable; }); return Object.assign({}, pool[rng(0, pool.length - 1)], { uid: Date.now() + Math.random() }); }
function getSellPrice(it) { return it.sellPrice || Math.max(5, Math.floor(it.price * 0.35)); }
function totalPotions(p) { return (p.small || 0) + (p.medium || 0) + (p.large || 0); }
function fmtCooldown(ms) { var s = Math.ceil(ms / 1000), m = Math.floor(s / 60); return m + ":" + (s % 60 < 10 ? "0" : "") + s % 60; }
function getPotionHeal(baseHeal, perks) { var bonus = 0; (perks || []).forEach(function (pid) { var p = PERKS.find(function (x) { return x.id === pid; }); if (p && p.potionBonus)
    bonus += p.potionBonus; }); return Math.floor(baseHeal * (1 + bonus)); }
function applyXpForLevelUp(prev, xpG) {
    ...
    if (!leveled)
        return { hero: nh, luInfo: null };
    var perkLv = null;
    for (var l = lv; l > prev.level; l--) {
        if (l % 3 === 0) {
            perkLv = l;
            break;
        }
